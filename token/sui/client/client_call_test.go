...
/*func TestClient_DryRunTransaction(t *testing.T) {
	...
	require.NoError(t, err)
	coin, err := coins.pickOneCoinForAmount(2000)
	require.NoError(t, err)
...
//	require.NoError(t, err)
//
//	firstCoin, err := coins.pickOneCoinForAmount(100)
//	require.NoError(t, err)
...
//	require.NoError(t, err)
//
//	firstCoin, err := coins.pickOneCoinForAmount(1000)
//	require.NoError(t, err)
//
...
//	coins, err := chain.GetSuiCoinsOwnedByAddress(context.TODO(), *Address)
//	require.NoError(t, err)
//	coin, err := coins.pickOneCoinForAmount(30000)
//	require.NoError(t, err)
//	type args struct {
//		ctx             context.Context
//		address         types.Address
...
func TestClient_DevInspectTransactionBlock(t *testing.T) {
	...
	coins, err := chain.GetSuiCoinsOwnedByAddress(context.TODO(), *Address)
	require.NoError(t, err)
	coin, err := coins.pickOneCoinForAmount(1000)
	require.NoError(t, err)
	tx, err := chain.TransferSui(context.TODO(), *Address, *Address, coin.CoinObjectId, 1000, 1000)
	require.NoError(t, err)
	type args struct {
