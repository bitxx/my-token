...
import (
	"fmt"
	"math/rand"
	"mytoken/core/lib/sublib/subkey/ecdsa"
	"mytoken/core/lib/sublib/subkey/ed25519"
	...
	for scheme, tests := range testsMap {
		for _, c := range tests {
			t.Run(fmt.Sprintf("%s-%s", scheme, c.uri), func(t *testing.T) {
				s, err := DeriveKeyPair(scheme, c.uri)
				if err != nil {
					assert.True(t, c.err)
					return
				}
				...
				pub := s.Public()
				assert.Equal(t, c.publicKey, EncodeHex(pub))
				if c.accountID != "" {
					assert.Equal(t, c.accountID, EncodeHex(s.AccountID()))
				}
				seed := EncodeHex(s.Seed())
				if s.Seed() == nil {
					seed = ""
				}
