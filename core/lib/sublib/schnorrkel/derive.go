...
func (pk *PublicKey) DeriveKey(t *merlin.Transcript, cc [ChainCodeLength]byte) (*ExtendedKey, error) {
	...
	// derivedPk = pk + (sc * g)
	p1 := ristretto255.NewElement().ScalarBaseMult(sc)
	p2 := ristretto255.NewElement()
	p2.Add(pk.key, p1)
...
func (pk *PublicKey) DeriveScalarAndChaincode(t *merlin.Transcript, cc [ChainCodeLength]byte) (*ristretto255.Scalar, [ChainCodeLength]byte, error) {
	if t == nil {
