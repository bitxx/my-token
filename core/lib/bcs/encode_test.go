...
func TestMarshal_optional(t *testing.T) {
	if _, err := Marshal(WrapperWithWrongOptional{}); err == nil {
	...
	optionalUnsetBytes, err := Marshal(optionalUnset)
	if err != nil {
		t.Error(err)
	}
	...
	s := "123"
	optionalSet.Outer = &s
	optionalSetBytes, err := Marshal(optionalSet)
	if err != nil {
		t.Error(err)
	}
